...
class MSSQLQueryGenerator extends AbstractQueryGenerator {
  ...
  describeTableQuery(tableName, schema, schemaDelimiter, dataBaseName) {
    let sql = [
      'SELECT',
      ...
      "COLUMNPROPERTY(OBJECT_ID(c.TABLE_CATALOG+'.'+c.TABLE_SCHEMA+'.'+c.TABLE_NAME), c.COLUMN_NAME, 'IsIdentity') as 'IsIdentity',",
      "prop.value AS 'Comment'",
      'FROM',
      'INFORMATION_SCHEMA.TABLES t',
      'INNER JOIN',
      'INFORMATION_SCHEMA.COLUMNS c ON t.TABLE_NAME = c.TABLE_NAME AND t.TABLE_SCHEMA = c.TABLE_SCHEMA AND t.TABLE_CATALOG = c.TABLE_CATALOG',
      ...
      "ON sc.object_id = object_id(t.TABLE_CATALOG+'.'+t.table_schema + '.' + t.table_name) AND sc.name = c.column_name",
    ...
    if (dataBaseName) {
      sql += ` AND t.TABLE_CATALOG =${wrapSingleQuote(dataBaseName)}`;
    }
  ...
  showIndexesQuery(tableName) {
    ...
    const dataBaseName = this.quoteIdentifier(tableName.dataBaseName);
    const table = this.quoteTable(tableName);
    return 'SELECT ind.name as index_name, col.name as column_name, ind.is_unique, ' +
      'ind.is_primary_key as is_primary_key, ind.is_unique_constraint,ic.is_descending_key ' +
      `FROM ${dataBaseName}.sys.indexes ind  ` +
      `INNER JOIN ${dataBaseName}.sys.index_columns ic ON ic.object_id = ind.object_id AND ic.index_id = ind.index_id ` +
      `INNER JOIN ${dataBaseName}.sys.columns col ON col.object_id = ic.object_id AND col.column_id = ic.column_id ` +
      `INNER JOIN ${dataBaseName}.sys.tables t ON t.object_id = ind.object_id ` +
