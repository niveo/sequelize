...
class QueryGenerator {
  ...
  addSchema(param) {
    ...
    return {
      ...
      dataBaseName: param._dataBaseName || (param.sequelize && param.sequelize.config ? param.sequelize.config.database : null),
      toString() {
    ...
    if (_.isObject(param)) {
      if (this._dialect.supports.schemas) {
        if (param.dataBaseName) {
          table += `${this.quoteIdentifier(param.dataBaseName)}.`;
        }
        if (param.schema) {
