...
class ConnectionManager {
    ...
    if (!config.replication) {
      this.pool = new Pool({
        ...
        destroy: async connection => {
          const res = await this._disconnect(connection);
  ...
  getConnection(options = {}) {
    ...
    return promise.then(async() => {
      try {
        const res = await this.pool.acquire(options.type, options.useMaster);
        debug('connection acquired');
        return res;
      } catch (err) {
        if (err instanceof TimeoutError) {
          throw new errors.ConnectionAcquireTimeoutError(err);
        }
      }
    });
  }
