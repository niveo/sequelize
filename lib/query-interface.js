...
class QueryInterface {
  ...
  dropTable(tableName, options) {
    ...
    return this.sequelize.query(sql, options).then(async() => {
      const promises = [];
  ...
  dropAllTables(options = {}) {
    ...
    const dropAllTables = async tableNames => {
      const dropOpts = { ...options, cascade: true };
      for (const tableName of tableNames) {
        // if tableName is not in the Array of tables names then don't drop it
        if (!skip.includes(tableName.tableName || tableName)) {
          await this.dropTable(tableName, dropOpts);
        }
      }
    };
  ...
  async delete(instance, tableName, identifier, options) {
    ...
    for (const cascade of cascades) {
      ...
      for (const destroyInstance of instances) {
        await destroyInstance.destroy();
      }
    }
    options.instance = instance;
    return this.sequelize.query(sql, options);
