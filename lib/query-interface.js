...
class QueryInterface {
  ...
  createTable(tableName, attributes, options, model) {
    ...
    if (
      ...
      tableName = this.QueryGenerator.addSchema({
        ...
        _dataBaseName:  !!model && model._dataBaseName || options.dataBaseName
      });
    }
      ...
      return this.getForeignKeysForTables(tableNames, options).then(foreignKeys => {
        ...
        tableNames.forEach(tableName => {
          ...
          if (_.isObject(tableName)) {
            normalizedTableName = `${tableName.dataBaseName}.${tableName.schema}.${tableName.tableName}`;
          }
  ...
  describeTable(tableName, options) {
    ...
    const sql = this.QueryGenerator.describeTableQuery(tableName, schema, schemaDelimiter, dataBaseName);
  ...
  getForeignKeysForTables(tableNames, options) {
    ...
    return Promise.map(tableNames, tableName =>
      ...
      tableNames.forEach((tableName, i) => {
        if (_.isObject(tableName)) {
          tableName = `${tableName.dataBaseName}.${tableName.schema}.${tableName.tableName}`;
        }
