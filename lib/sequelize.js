...
class Sequelize {
  ...
  async query(sql, options) {
    ...
    return retry(async retryParameters => {
      ...
      const connection = options.transaction
        ...
        : await this.connectionManager.getConnection(options);
  ...
  async sync(options) {
    ...
    if (!models.length) await this.authenticate(options);
  ...
  async transaction(options, autoCallback) {
    ...
    try {
      ...
      try {
        if (!transaction.finished) return await transaction.rollback().catch(() => {});
      } finally {
