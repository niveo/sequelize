...
class InstanceValidator {
  ...
  _validate() {
    ...
    return Promise.all([
      this._perAttributeValidators().catch((err) => { console.error(err)}),
      this._customValidators().catch((err) => { console.error(err) })
    ]).then(() => {
   ...
   *
   * @returns {Promise<Array.<Object>>} Promise reflections.
   * @private
   */
  _perAttributeValidators() {
    ...
    _.forIn(this.modelInstance.rawAttributes, (rawAttribute, field) => {
      ...
      if (this.modelInstance.validators.hasOwnProperty(field)) {
        validators.push(reflectPromise(this._singleAttrValidate(value, field, rawAttribute.allowNull)));
      }
    });
   ...
   *
   * @returns {Promise<Array.<Object>>} Reflected promises
   * @private
   */
  _customValidators() {
    const validators = [];
    _.each(this.modelInstance._modelOptions.validate, (validator, validatorType) => {
      ...
      validators.push(reflectPromise(this._invokeCustomValidator(validator, validatorType).catch(() => { })));
    });
  ...
  _singleAttrValidate(value, field, allowNull) {
    ...
    _.forIn(this.modelInstance.validators[field], (test, validatorType) => {
      ...
      if (typeof test === 'function') {
        validators.push(reflectPromise(this._invokeCustomValidator(test, validatorType, true, value, field)));
        return;
      }
      ...
      validators.push(reflectPromise(validatorPromise));
    });
  ...
  async _invokeCustomValidator(validator, validatorType, optAttrDefined, optValue, optField) {
    ...
    try {
      ...
      this._pushError(false, errorKey, e, optValue, validatorType);
    }
  }
