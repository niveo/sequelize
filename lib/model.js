...
class Model {
  ...
  static async sync(options) {
    ...
    await this.QueryInterface.createTable(this.getTableName(options), attributes, options, this)
      ...
      .then(indexes => {
        ...
        return Promise.all(indexes.map(index => this.QueryInterface.addIndex(
  ...
  static async _findSeparate(results, options) {
    ...
    await Promise.all(options.include.map(include => {
      if (!include.separate) {
  ...
  static async upsert(values, options) {
    ...
    if (options.returning === true && primaryKey) {
      res = await this.findByPk(primaryKey, options).then(record => [record, created]);
    } else {
      res = created;
    }
    if (options.hooks) {
      return this.runHooks('afterUpsert', res, options);
    }
  }
   ...
   *
   * If validation fails, the promise is rejected with an AggregateError.
   *
  ...
  static async bulkCreate(records, options = {}) {
    ...
    // Validate
    if (options.validate) {
      const errors = new sequelizeErrors.AggregateError();
      ...
      await Promise.all(instances.map(async instance => {
        await instance.validate(validateOptions)
          .catch(err => {
            errors.add(new sequelizeErrors.BulkRecordError(err, instance));
          });
        delete options.skip;
    ...
    const results = await this.QueryInterface.bulkInsert(this.getTableName(options), records, options, fieldMappedAttributes);
  ...
  static async destroy(options) {
    ...
    if (this._timestampAttributes.deletedAt && !options.force) {
      ...
      await this.QueryInterface.bulkUpdate(this.getTableName(options), attrValueHash, Object.assign(where, options.where), options, this.rawAttributes);
    }
    await this.QueryInterface.bulkDelete(this.getTableName(options), options.where, options, this);
  ...
  static async restore(options) {
    ...
    const res = this.QueryInterface.bulkUpdate(this.getTableName(options), attrValueHash, options.where, options, this.rawAttributes);
    // Run afterDestroy hook on each record individually
    if (options.individualHooks) {
  ...
  static async update(values, options) {
    ...
    if (options.validate) {
      ...
      await build.validate(options).then(attributes => {
  ...
  async save(options) {
    ...
    if (wasNewRecord && this._options.include && this._options.include.length) {
      await Promise.all(this._options.include.filter(include => !(include.association instanceof BelongsTo ||
  ...
  */
  ...
  async destroy(options) {
    ...
    const res = this.constructor.QueryInterface.delete(this, this.constructor.getTableName(options), where, Object.assign({ type: QueryTypes.DELETE, limit: null }, options));
    // Run after hook
    if (options.hooks) {
  ...
  async restore(options) {
    ...
    const res = await this.save({ ...options, hooks: false, omitNull: false });
    // Run after hook
    if (options.hooks) {
